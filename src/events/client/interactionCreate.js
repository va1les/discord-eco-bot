...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (!await client.db.user.findOne({ gid: interaction.guild.id, uid: interaction.user.id })) await client.db.user.create({ gid: interaction.guild.id, uid: interaction.user.id })
        ...
        if (timestamps.has(interaction.user.id)) {
            ...
            if (now < expirationTime) {
                ...
                return interaction.reply(`Подождите **${timeLeft.toFixed(2)}с.** и используйте команду ещё раз.`);
            }
        }
